...
/**
 * Reviewer Component - Comprehensive Styling Object
 * 
 * This object contains all the styling for the reviewer review page.
...
const styles = {
  ...
  // Textarea for reviewer comments and notes
  textarea: dark => ({
    width: '90%',
    padding: '.85rem 1rem',
    marginBottom: '1rem',
 ...
 * Reviewer Component
 * 
 * This component handles the final reviewer stage of the PDF workflow system.
 * Final reviewers can:
 * - View and select Stage 2 submissions from librarians
 * - Download and preview PDFs
 * - Add comments and notes
 * - Approve submissions to send to admin for publication
 * - Return submissions to librarians for corrections
...
export default function Reviewer() {
  ...
  const [notesCache, setNotesCache] = useState(() => {
    const saved = localStorage.getItem('reviewerNotesCache');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  // Access control: verify user is authenticated as a reviewer
  useEffect(() => {
    ...
    if ((role !== 'reviewer' && role !== 'finalreviewer') || Date.now() > exp) {
      window.alert('Unauthorized'); // Show error if not reviewer or session expired
  ...
  useEffect(() => {
    ...
    localStorage.setItem('reviewerNotesCache', JSON.stringify(notesCache));
  ...
  useEffect(() => {
    ...
    if (activeTab === 'to-review') {
      ...
      data = data.filter(s => s.stage === 'Stage3' && s.approvedBy === user);
  ...
  const selectSubmission = (s, idx) => {
    ...
    if (s.stage === 'Stage2' && s.returnedFromReview) {
      targetTab = 'returned';
    } else if (s.stage === 'Stage2' && !s.returnedFromReview) {
      targetTab = 'to-review';
    } else if (s.stage === 'Stage3' && s.approvedBy === user) {
      targetTab = 'sent';
    ...
    // Notes will be automatically loaded from cache via getCurrentNotes()
  };

  // Approve submission
  const approveSubmission = async () => {
    if (!selected) return window.alert('Select one');
    if (confirmOn && !window.confirm('Approve and send to Admin for publication?')) return;
    ...
    const approvedSubmission = { 
      ...
      reviewerNotes: notesCache[selected.filename] || ''
    };
    updatedSubs.push(approvedSubmission);
    
    // Remove from previous user's history (librarian who sent it)
    if (selected.sentBy) {
      // This document will no longer appear in the librarian's "sent" tab
      // as it's now in Stage3 and approved by reviewer
      // The filtering logic in getFilteredDataForTab will handle this automatically
      // since the document now has stage 'Stage3' and approvedBy set to current reviewer
    }
    ...
    adminLog.push({
      ...
      notes: `Approved by ${currentUser} and sent to Admin for publication`,
      action: 'approved'
    });
    ...
    notifications.push({
      ...
      targetStage: 'Stage3',
      time: Date.now(),
      message: `${selected.filename} has been approved and is ready for publication.`
    });
    ...
    setSuccessMsg('Approved and sent to Admin for publication!');
    setTimeout(() => setSuccessMsg(''), 5000);
    window.alert('Approved and sent to Admin for publication!');
  };

  // Send back to librarian
  const sendBackToLibrarian = async () => {
    if (!selected) return window.alert('Select one');
    ...
    updatedSubs.push(returnedSubmission);
    
    // Remove from previous user's history (librarian who sent it)
    if (selected.sentBy) {
      // This document will no longer appear in the librarian's "sent" tab
      // as it's now returned and will appear in "returned" tab
      // The filtering logic in getFilteredDataForTab will handle this automatically
      // since the document now has stage 'Stage2' and returnedFromReview set to true
    }
    ...
    adminLog.push({
      ...
      stage: 'RETURNED',
      filename: selected.filename,
      notes: `Returned to librarian by ${currentUser}`,
      action: 'returned'
    });
    localStorage.setItem('adminLog', JSON.stringify(adminLog));
    ...
    setFileInputKey(Date.now());
    setSuccessMsg('Returned to librarian!');
    setTimeout(() => setSuccessMsg(''), 5000);
    window.alert('Returned to librarian!');
  };

  // Undo submission - move from sent back to to-review
  const undoSubmission = async () => {
    ...
    // Move from Stage3 back to Stage2 (to-review)
    ...
    const undoneSubmission = { 
      ...
      filename: newFilename,
      time: Date.now(),
      approvedBy: null, // Remove approval
      readyForPublication: false, // Remove publication flag
      returnedFromReview: false // Ensure it's not marked as returned
    };
                ...
                <>
                  <div style={{
                    ...
                    <div style={{ marginTop: 'auto', paddingTop: 20, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {(activeTab === 'to-review' || activeTab === 'returned') && (
                        <>
                          <button
                      ...
                      {selected && selected.stage === 'Stage3' && selected.approvedBy === atob(sessionStorage.getItem('authUser') || '') && (
                        <button
